 ...
 * */
	...
	public DataSet editedKNN(DataSet train){
		DataSet editedTrain=new DataSet();
		editedTrain.name=train.name;
		editedTrain.attributes=train.attributes;
		for(int i=0;i<train.examples.size();i++){
			Example newExample=train.examples.get(i);
			if(i<k){
				editedTrain.examples.add(newExample);
			}
			else{
	...
	*/
	...
	PriorityQueue<DistanceExample> findNeighbor(Example e1){
		...
		for(Example ex: exs){
			...
			//System.out.println("example="+e1+"\nclosest example="+priorityQueue.peek().example+"\n	distance="+this.getDistance(e1, priorityQueue.peek().example));
			System.out.println("distance="+this.getDistance(e1, priorityQueue.peek().example));
		}
	...
	public static void main(String[] args) throws Exception{
		...
		if(arrArgs.contains("-T")){
			...
			TrainTestSets t1=new TrainTestSets(datasetArgs);
			//t1.train=c1.editedKNN(t1.train);
			System.out.println("# of examples="+t1.train.examples.size());
